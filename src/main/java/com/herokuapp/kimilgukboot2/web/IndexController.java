...
public class IndexController {
	...
	public String signupGet(Model model) {
		return "signup";// signup.머스테치 파일을 호출한다.
	}

	@PostMapping("/signup") // 회원가입 API실행
	public String signupPost(HttpServletResponse response, SimpleUsersDto requestDto) throws IOException {
		...
		if (usersDto == null) {// 중복ID가 없다
			// 해킹방지용 보안코드 추가
			requestDto.setRole("USER");// 회원가입로직에서는 강제로 권한을 USER로 준다.
			simpleUsersService.save(requestDto);
			ScriptUtils.alertAndMovePage(response, "회원가입이 완료되었습니다. 로그인 해주세요", "/");
		} else {
			// 중복ID가 있다면 진행 금지
	...
	public String kakaoMap(@RequestParam(value = "keyword", defaultValue = "천안시") String keyword, Model model)
			throws IOException {
		// 다음시간에 아래 null 자리에 전기자동차 충전소 위치정보를 받아서 바인딩 할 예정.
		// 샘플코드 시작
		...
		urlBuilder.append(
				"&" + URLEncoder.encode("addr", "UTF-8") + "=" + URLEncoder.encode(keyword, "UTF-8")); /* 검색대상 충전소주소 */
		...
		/*
		 * 의령군 전기차 충전소 일때 JSONObject rfcOpenApi = (JSONObject)
		 * (jsonObject.get("rfcOpenApi")); JSONObject header = (JSONObject)
		 * rfcOpenApi.get("header"); JSONObject body = (JSONObject)
		 * rfcOpenApi.get("body");
		 */
		...
		return "kakaomap";// kakaomap.mustach 파일을 불러온다.
	}
	...
	public String postsUpdate(HttpServletResponse response,@PathVariable Long id, Model model, @LoginUser SessionUser user) throws IOException {
		...
		if(!user.getName().equals(dto.getAuthor()) && !"ROLE_ADMIN".equals(user.getRole())) {
			ScriptUtils.alertAndBackPage(response, "본인글만 수정 가능합니다.");
		}
		// 멀티첨부파일 출력부분
